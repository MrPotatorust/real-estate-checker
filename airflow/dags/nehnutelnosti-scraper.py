...
for sale_rent_bool in [1, 2, 3, 4]:
    ...
    for cities in enumerate(slovak_cities):
        ...
        while not soup.find(string="Nenašli sa žiadne výsledky"):
            ...
            counter += 1
            html_doc = getHtmlDoc(f"https://www.nehnutelnosti.sk/{cities[1]}/{house_apartment_text}/{sale_rent_text}/?p[page]={counter}")
            soup = BeautifulSoup(html_doc, 'lxml')
            logger.info(f"https://www.nehnutelnosti.sk/{cities[1]}/{house_apartment_text}/{sale_rent_text}/?p[page]={counter}")
